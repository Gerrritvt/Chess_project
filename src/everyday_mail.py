...
def send_simple_message(mail_api_url:str, key:str, to:str, subject:str, text:str):
    """Send a text email.
...
def yesterday_message(games:Generator, username:str) -> str:
    ...
    for game in games:
        ...
        if (time.day, time.month, time.year) == yesterday_dmj:
            ...
            if 'opening' in game.keys():
                made_mistake, move_nr = opening_mistake(game)
                if made_mistake:
...
def opening_mistake(game:dict) -> tuple[bool, int]:
...
if __name__ == "__main__":
    ...
    send_simple_message(mail_api_url, key, to, subject, text)
